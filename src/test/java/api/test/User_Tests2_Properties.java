...
public class User_Tests2_Properties {
	Faker fs;
	User userPayload;
	String username;
	String firstName;
	...
	public void TC01_testCreateUser()
	{
		...
		Response response = UserEndPointsProperties.createUser(userPayload, user);
	...
	{
		String user = this.userPayload.getUsername();
		Response response = UserEndPointsProperties.readUser(user);
		System.out.println("***********************************");
	...
	public void TC03_testUpdateUser()
	{
		...
		userPayload.setEmail(fs.internet().safeEmailAddress());
		String user = this.userPayload.getUsername();
		Response response = UserEndPointsProperties.updateUser(userPayload, user);
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200);
	...
	{
		String user = this.userPayload.getUsername();
		Response response = UserEndPointsProperties.deleteUser(user);
		Assert.assertEquals(response.getStatusCode(), 200);
